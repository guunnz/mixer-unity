...
public class MonsterGeneUtils
{
    ...
    public static Cls GetMonsterClass(string hex)
    {
        string hexBin = "";
        hex = hex.Replace("0x", "");
        hexBin = String.Join(String.Empty,
    ...
    public static (int, int, int, int) GetStats(List<string> classCombo, MonsterClass monsterClass)
    {
        float hp, speed, skill, morale;
        hp = speed = skill = morale = 0f;


        switch (monsterClass.ToString().ToLower())
        {
            case "beast":
                hp = -1;
                skill = -1;
    ...
    public static List<string> GetMonsterPartsClasses(string genes)
    {
        List<string> parts = MonsterGeneUtils.ParsePartIdsFromHex(genes);
    ...
    public static GetMonstersExample.Stats GetStatsByGenesAndMonsterClass(string genes, MonsterClass monsterClass)
    {
        List<string> parts = MonsterGeneUtils.ParsePartIdsFromHex(genes);
        ...
        var stats = MonsterGeneUtils.GetStats(extractedParts, monsterClass); // Update this line if needed to pass the correct parts

        return new GetMonstersExample.Stats()
