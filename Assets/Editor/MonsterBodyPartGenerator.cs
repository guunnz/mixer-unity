...
public static class MonsterBodyPartsGenerator
{
    [MenuItem("Monster/Generate Body Parts Manager")]
    public static void GenerateBodyPartsManager()
    {
        var manager = ScriptableObject.CreateInstance<MonsterBodyPartsManager>();
        manager.monsterBodyParts = new List<MonsterBodyPart>();
            ...
            var skillToBodyPartMap = new Dictionary<SkillName, BodyPart>
        {
            ...
            { SkillName.MonsterKiss, BodyPart.Mouth },
        ...
        {
            var bodyPart = ScriptableObject.CreateInstance<MonsterBodyPart>();
            bodyPart.skillName = pair.Key;
            bodyPart.bodyPart = pair.Value;

            manager.AddBodyPart(bodyPart);

            // Save each MonsterBodyPart as a ScriptableObject in the project
            AssetDatabase.CreateAsset(bodyPart, $"Assets/MonsterBodyParts/{pair.Key}_{pair.Value}.asset");
        }

        // Save the MonsterBodyPartsManager
        AssetDatabase.CreateAsset(manager, "Assets/MonsterBodyPartsManager.asset");
